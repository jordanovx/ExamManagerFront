...
class AddTimeTablePage extends React.Component {
  ...
  isAllowedExtension = extension => {
    return (
      this.state.allowed_extensions.filter(element => {
  ...
  handleDrop = (files, event) => {
    ...
    if (!this.isAllowedExtension(extension)) {
      this.setState({
        warning_message:
          "Екстензијата на фајлот не е дозволена! Дозволени се (.xls .xlsx .csv)"
      });
  ...
  objectExtensionMap = extension => {
    ...
    return map.get(extension) === undefined
      ? map.get("unknown")
      : map.get(extension);
  };
  getDefaultMessage = limit => {
    return (
      <div>
        <p> {"Прикачи фајл"} </p> <p> {"Лимит " + limit + " MB"}</p>
      </div>
    );
  };
      ...
      <div>
        ...
        <div className="wrapper">
          <Col xs={6} className="col-md-offset-3">
            ...
            {!this.state.file_not_allowed &&
            this.state.file_count !== undefined ? (
          ...
          <div id="react-file-drop">
            <FileDrop onDrop={this.handleDrop} onDragOver={this.handleDragOver}>
              {!(this.state.file_count > 0) ? (
                this.getDefaultMessage(10)
              ) : (
                <img
                  src={this.objectExtensionMap(this.state.file_extension)}
                  alt=""
                />
              )}
              ...
              {this.state.file_size > 0
                ? this.calculateFileSize(this.state.file_size)
                : ""}
            </FileDrop>
          </div>
          <Button variant={"primary"} className="submit-file" disabled>
            {" "}
            Продолжи{" "}
          </Button>
        </div>
